...
async function createSchema(name, displayName, definition) {
  try {
    await request(`${BASE_URL}/_system/schemas/personal`, 'POST', {
      name,
      display_name: displayName,
      definition,
    });
    console.log(`✅ Schema ${name} 创建成功`);
  } catch (error) {
    console.log(`⚠️ Schema ${name} 创建失败或已存在:`, error.message);
  }
}
...
async function main() {
  console.log('开始创建Schema...');
  
  await createSchema('articles', '文章', {
    fields: [
  ...
  await createSchema('projects', '项目', {
    fields: [
  ...
  await createSchema('experiences', '工作经历', {
    fields: [
  ...
  await createSchema('educations', '教育背景', {
    fields: [
